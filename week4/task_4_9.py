...
class Agent:
    def __init__(self, env, **kwargs):
        ...
        self.greedy_actions = kwargs.get(
            "greedy_actions", np.ones(self.env.target + 1).astype(int)
        )
    ...
    def estimate_action(self, bank: int, stake: int) -> float:
        ...
        action_value = (self.env.p * v_win) + ((1.0 - self.env.p) * v_lose)
        return action_value
    ...
    def estimate_states(self, eps: float) -> float:
        ...
        for state in self.states[1:-1]:
            ...
            for stake in possible_actions:
                ...
                action_prob = (
                    (1.0 - eps)
                    if stake == self.greedy_actions[state]
                    else (eps / (len(possible_actions) - 1))
                )
